...
const criteriaLines = (m = {}) => {
  const rows = [];
  Object.entries(m.detalhes || {}).forEach(([id, r]) => { if (typeof r?.atendeu === 'boolean') rows.push({ id, titulo: r.titulo || id, atendeu: r.atendeu, evidencia: r.evidencia || '' }); });
...
const makePdf = (m, supervisor) => {
  ...
  const line = (text, size = 9, bold = false, color = [30, 41, 59], gap = 5) => {
    ...
    pdf.splitTextToSize(String(text || '—'), pageW - margin * 2).forEach((p) => { if (y > 278) { pdf.addPage(); y = 20; } pdf.text(p, margin, y); y += gap; });
  };
  ...
  section('Resumo da monitoria'); line(m.feedback || m.resumo || 'Sem resumo registrado.', 9, false, [51, 65, 85], 5);
  const rows = criteriaLines(m);
  if (rows.length) { section('Critérios avaliados'); rows.forEach((r, i) => line(`${i + 1}. ${r.titulo || r.id} — ${r.atendeu ? 'ATENDEU' : 'NÃO ATENDEU'}${r.evidencia ? ` — ${r.evidencia}` : ''}`, 8.5, false, r.atendeu ? [22, 101, 52] : [185, 28, 28], 5)); }
  ...
  section('Retorno solicitado'); line('Solicitamos a análise da monitoria e o retorno do supervisor sobre a orientação realizada, ciência dos pontos identificados e próximos passos para acompanhamento.', 9, false, [51, 65, 85], 5);
...
export default function SupervisorCommunicationView({ monitorias = [], supervisores = [], onUpdateSupervisores, onUpdateMonitoria, darkMode = false, showToast }) {
  ...
  const [enviando, setEnviando] = useState(false); const [progresso, setProgresso] = useState('');
  ...
  const salvarSupervisor = async () => {
    ...
    setForm({ id: '', nome: '', email: '', departamento: '' }); showToast?.('Supervisor salvo.', 'success');
  };

  const excluirSupervisor = async (id) => { if (!window.confirm('Desativar este supervisor?')) return; await onUpdateSupervisores(supervisores.map((s) => s.id === id ? { ...s, ativo: false } : s)); };
  ...
  return (
    <div className={`flex-1 overflow-y-auto p-6 md:p-8 space-y-6 ${darkMode ? 'bg-[#0B1117] text-white' : 'bg-[#F5F7F8] text-slate-900'}`}>
      <header className="flex flex-col xl:flex-row xl:items-end justify-between gap-4"><div><span className="text-[9px] font-black uppercase tracking-[.22em] text-blue-500">Comunicação • ciclo de acompanhamento</span><h1 className="text-3xl font-black mt-2">Supervisores & retornos</h1><p className="text-xs text-slate-400 mt-1">Cadastre responsáveis, envie as monitorias e acompanhe o retorno sem precisar escrever e-mail manualmente.</p></div>{aba === 'envios' && <button disabled={enviando} onClick={enviarTodos} className="px-5 py-3 rounded-2xl bg-blue-600 text-white text-xs font-black disabled:opacity-50">{enviando ? progresso || 'Enviando...' : `Enviar todas as pendentes (${pendentes.length})`}</button>}</header>
      <div className="flex gap-2 flex-wrap"><button onClick={() => setAba('envios')} className={`px-4 py-2 rounded-xl text-xs font-black ${aba === 'envios' ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-600'}`}>Monitorias</button><button onClick={() => setAba('cadastro')} className={`px-4 py-2 rounded-xl text-xs font-black ${aba === 'cadastro' ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-600'}`}>Supervisores ({activeSup.length})</button><button onClick={() => setAba('gmail')} className={`px-4 py-2 rounded-xl text-xs font-black ${aba === 'gmail' ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-600'}`}>Script Gmail</button></div>
      {aba === 'gmail' && <GmailScriptSettingsView darkMode={darkMode} showToast={showToast} />}
      {aba === 'cadastro' && <div className="space-y-5"><section className={card}><h2 className="text-base font-black">Cadastrar supervisor</h2><p className="text-[10px] text-slate-400 mt-1 mb-4">O departamento será usado para identificar automaticamente o responsável nas monitorias.</p><div className="grid grid-cols-1 md:grid-cols-3 gap-3"><input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} placeholder="Nome do supervisor" className="border rounded-xl px-3 py-3 text-xs bg-transparent" /><input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="E-mail corporativo" className="border rounded-xl px-3 py-3 text-xs bg-transparent" /><select value={form.departamento} onChange={(e) => setForm({ ...form, departamento: e.target.value })} className="border rounded-xl px-3 py-3 text-xs bg-transparent"><option value="">Departamento</option>{['Filiais - Comercial','Filiais - Financeiro','Filiais - Pendência','Unidades','Embarcadoras'].map((d) => <option key={d}>{d}</option>)}</select></div><div className="flex justify-end mt-4"><button onClick={salvarSupervisor} className="px-5 py-2.5 rounded-xl bg-blue-600 text-white text-xs font-black">{form.id ? 'Atualizar supervisor' : 'Cadastrar supervisor'}</button></div></section><section className={card}><h2 className="text-base font-black mb-3">Supervisores cadastrados</h2><div className="space-y-2">{activeSup.map((s) => <div key={s.id} className="flex items-center gap-3 p-3 rounded-xl border"><div className="w-9 h-9 rounded-xl bg-blue-500/10 text-blue-500 grid place-items-center font-black">{s.nome.slice(0,1).toUpperCase()}</div><div className="flex-1 min-w-0"><div className="text-xs font-black truncate">{s.nome}</div><div className="text-[10px] text-slate-400 truncate">{s.email} • {s.departamento}</div></div><button onClick={() => setForm(s)} className="text-[10px] font-black text-blue-500">Editar</button><button onClick={() => excluirSupervisor(s.id)} className="text-[10px] font-black text-rose-500">Desativar</button></div>)}</div></section></div>}
