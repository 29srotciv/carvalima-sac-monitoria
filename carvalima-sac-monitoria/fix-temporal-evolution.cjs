...
if (s.includes(MARK)) {
  console.log('Evolução temporal V2 já aplicada.');
  process.exit(0);
}
...
if (s.includes("const dateOf = (m) => m?.dataAtendimento || ''")) {
  s = s.replace("const dateOf = (m) => m?.dataAtendimento || '';", helper);
} else if (!s.includes('const normalizeDate =')) {
  throw new Error('Âncora de dateOf não encontrada.');
}
...
if (temporalRegex.test(s)) {
  s = s.replace(temporalRegex, '\n' + block);
} else {
