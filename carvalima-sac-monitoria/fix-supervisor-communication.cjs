...
const MARK = 'CARVALIMA_SUPERVISOR_COMMUNICATION_V4';
if (text.includes(MARK)) process.exit(0);

const reactImport = "import React, { useState, useMemo, useEffect, useRef } from 'react';";
const importMarker = "import SupervisorCommunicationView from './SupervisorCommunicationView';";
if (!text.includes(importMarker)) text = text.replace(reactImport, reactImport + "\n" + importMarker);
...
if (!text.includes('const handleUpdateSupervisores = async')) {
  ...
  const handler = `  const handleUpdateSupervisores = async (items) => {\n    const next = Array.isArray(items) ? items : [];\n    setSupervisores(next);\n    localStorage.setItem('carvalima_qa_supervisores', JSON.stringify(next));\n  };\n\n`;
  if (text.includes(anchor)) text = text.replace(anchor, handler + anchor);
}
