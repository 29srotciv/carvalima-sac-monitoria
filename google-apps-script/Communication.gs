...
function carvalimaAuth_(payload) {
  var cfg = carvalimaConfig_();
  if (!payload || payload.tokenSeguranca !== cfg.token) throw new Error('Token de segurança inválido.');
  return cfg;
}

function carvalimaBook_(cfg) { return SpreadsheetApp.openById(cfg.spreadsheetId); }

function carvalimaSheet_(book, name, headers) {
...
function carvalimaFindRow_(sheet, column, value) {
...
function carvalimaUpsert_(sheet, keyColumn, key, rowValues) {
  var row = carvalimaFindRow_(sheet, keyColumn, key);
...
function doPost(e) {
  try {
    ...
    return carvalimaJson_({ success: false, error: err.message || String(err) });
  }
}
...
function carvalimaSendSupervisorEmail_(p) {
  var cfg = carvalimaAuth_(p);
  var book = carvalimaBook_(cfg);
  ...
  var bodyText = [
    ...
    'Encaminhamos para sua ciência e retorno a monitoria de atendimento realizada pelo SAC.', '',
    'Colaborador: ' + (m.agente || '—'), 'Departamento: ' + (m.departamento || '—'), 'Unidade: ' + (m.unidade || '—'), 'Monitoria: ' + (m.id || '—'), 'Nota: ' + (m.nota || '—'), '',
    'Solicitamos a análise da avaliação, a orientação do colaborador quando aplicável e o registro do retorno pelo link abaixo:', p.responseUrl, '',
    'O retorno ficará registrado para acompanhamento da evolução e dos planos de ação do SAC.', '',
    'Atenciosamente,', 'Victor Silva', 'Analista SAC — Carvalima'
  ].join('\n');

  var bytes = Utilities.base64Decode(p.pdfBase64);
  var blob = Utilities.newBlob(bytes, MimeType.PDF, 'Carvalima_Monitoria_' + m.id + '.pdf');
...
function carvalimaSync_(p) {
  ...
  var incoming = Array.isArray(p.monitorias) ? p.monitorias : [];
  var created = 0; var updated = 0;
  incoming.forEach(function(m) {
    if (!m || !m.id) return;
    var row = carvalimaFindRow_(sh, 1, m.id);
    var payload = JSON.stringify(m);
    if (row < 0) { sh.appendRow([m.id, payload, new Date(m.updated_at || Date.now())]); created++; }
    else { sh.getRange(row, 2, 1, 2).setValues([[payload, new Date(m.updated_at || Date.now())]]); updated++; }
  });
  ...
  if (sh.getLastRow() >= 2) {
    sh.getRange(2, 1, sh.getLastRow() - 1, 3).getValues().forEach(function(r) {
      if (!r[1]) return;
      try { out.push(JSON.parse(r[1])); } catch (e) {}
    });
  }

  var env = book.getSheetByName('CARVALIMA_ENVIO_SUPERVISOR');
  if (env && env.getLastRow() >= 2) {
    var ev = env.getRange(2, 1, env.getLastRow() - 1, 14).getValues();
    var byId = {};
    ...
    out = out.map(function(m) {
      ...
      return Object.assign({}, m, {
        supervisorNome: r[2], supervisorEmail: r[3], supervisorDepartamento: r[4], supervisorToken: r[0],
        supervisorEnvioAt: r[7] ? new Date(r[7]).toISOString() : m.supervisorEnvioAt,
        supervisorStatus: r[13] || m.supervisorStatus,
        supervisorRetornoAt: r[9] ? new Date(r[9]).toISOString() : m.supervisorRetornoAt,
        feedbackSupervisor: r[11] || m.feedbackSupervisor,
        statusFeedback: r[13] === 'Respondido' ? 'Concluído' : m.statusFeedback,
        feedbackSupervisorAt: r[9] ? new Date(r[9]).toISOString() : m.feedbackSupervisorAt,
        decisaoSupervisor: r[10] || m.decisaoSupervisor,
        proximoPassoSupervisor: r[12] || m.proximoPassoSupervisor
      });
    });
  }
